...
i have implemented a simple version of a communication
system.
...
i have taken the input as an input file, operations are performed, and will print
...
Main Class is for the general input-output operations. we are going to read an
input file that contains directions about the simulation and do several actions,
then will print out the desired results into the output file. The name of the input and output files
will be given as arguments of the program. For the input-output test cases, we are
going to use main class.
...
we are going to store the corresponding data in these arrays. All of these arrays
...
network cost per MB.
- Before the actions, we should check the limit of the bill, if someone would
...
- we can declare extra fields or extra methods. 
...
we are going to read the input file line by line or token by token. The first 3 lines
...
should be on his/her ID. Also, there is no operation to create a bill object, therefore we
need to create in the Customer class and store the bill object whenever we are going
...
we should calculate the following and print them out to the output file.

1. For each operator, we should print out, the amount of time that they serviced for
